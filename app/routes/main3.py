...
──────────────
• Scraper scheduling
  - On app startup (init_scheduler) an APScheduler BackgroundScheduler is started.
  - Every 60 s a lightweight _tick() function checks whether any category is due
    to run based on its saved schedule (interval / daily / weekly).
  - When 'Run All Together' (sync_all_schedules) is on, only the global schedule
    is checked; all active categories run together.
  - When 'Run All Together' is off, each active category has its own schedule.

• Auto-publish vs Manual
  - publish_mode = 'auto'   → scraper inserts directly into published_news,
                               then immediately triggers PDF generation + email.
  - publish_mode = 'manual' → scraper inserts into non_published_news (default).

• Schedule storage
  - Each category's schedule is stored as JSON in user_settings columns
    schedule_all, schedule_agricultural, schedule_weather,
    schedule_financial, schedule_energy, schedule_global.
  - last_run times are stored in schedule_last_run_<cat> columns (TEXT / DATETIME).
  - The POST handler in user_settings() parses the form and serialises JSON.
"""
...
# ════════════════════════════════════════════════════════════════
#  DB / CONFIG HELPERS  (request-context safe)
# ════════════════════════════════════════════════════════════════
...
def _load_settings_raw(db_cfg: dict) -> dict:
    """Load user_settings row outside a Flask context."""
    try:
...
def _parse_schedule(row: dict, cat_key: str) -> dict:
    """
    Read schedule_<cat_key> column (JSON string) and return a dict like:
      {'type': 'interval', 'interval_hours': '2', 'interval_unit': 'hours'}
    Returns {} if not set.
    """
...
def _is_due(sched: dict, last_run_str: str) -> bool:
    """
    Return True if a category is due to run now based on its schedule
    and the timestamp of its last run.

    sched keys:
      type            : 'interval' | 'daily' | 'weekly'
      interval_hours  : str int
    ...
    if stype == "interval":
        n    = int(sched.get("interval_hours", 1))
        ...
        try:
            ...
            # Due if current time has reached the target and we haven't run today
            if now < target:
                return False
            if not last_run_str:
                return True
        ...
        try:
            ...
            if not last_run_str:
                return True
            last = datetime.fromisoformat(last_run_str)
            # Same calendar week check
            last_week_start = (last - timedelta(days=last.weekday())).date()
            now_week_start  = (now  - timedelta(days=now.weekday())).date()
            return last_week_start < now_week_start
        except Exception:
            return False
...
def _update_last_run(db_cfg: dict, cat_key: str):
    """Write now() into schedule_last_run_<cat_key> column."""
...
# ════════════════════════════════════════════════════════════════
#  BACKGROUND: SCRAPER  (manual or auto publish)
# ════════════════════════════════════════════════════════════════
...
def _bg_scraper(db_cfg: dict, instance_path: str,
    ...
    """
    Full scrape cycle.
    If publish_mode='auto', auto-publishes matched articles and kicks off
    PDF generation + email in its own sub-thread.
    """
    global _scraper_state
    with _scraper_lock:
        _scraper_state.update({
            "running": True,
            "message": f"Scraper running… (mode={publish_mode}, cats={target_categories})",
            "success": None,
        })
    try:
        ...
        # Auto-publish: trigger PDF + email for newly published articles
        if publish_mode == "auto":
            ...
            if pub_ids and mail_cfg and template_dir and pdf_folder and settings:
                # Fetch full article rows from published_news for email template
                try:
                    conn  = _raw_conn(db_cfg)
                    cur   = conn.cursor(dictionary=True)
                    ...
                    print(f"[BG-AUTO] Could not fetch pub articles for email: {e}")
                    pub_articles = raw_arts  # fallback
...
# ════════════════════════════════════════════════════════════════
#  SCHEDULER TICK  (runs every 60 s in a daemon thread)
# ════════════════════════════════════════════════════════════════

def _scheduler_tick(app):
    """
    Called every 60 s. Checks per-category schedules and fires the scraper
    for any category (or global) that is due.
    Must push an app context because it uses current_app indirectly via helpers.
    """
    with app.app_context():
        try:
            ...
            # Collect what to scrape this tick
            to_scrape: list = []   # list of category key lists

            if sync_all:
                sched    = _parse_schedule(settings, "global")
                last_run = settings.get("schedule_last_run_global") or ""
                if _is_due(sched, last_run):
                    to_scrape.append(("global", active_cats))
            else:
                for cat in active_cats:
                    sched    = _parse_schedule(settings, cat)
                    col_key  = f"schedule_last_run_{cat}"
                    last_run = settings.get(col_key) or ""
                    if _is_due(sched, last_run):
                        to_scrape.append((cat, [cat] if cat != "all" else ["all"]))

            if not to_scrape:
                return

            # Only start if scraper not already running
            with _scraper_lock:
                if _scraper_state["running"]:
                    print("[SCHEDULER] Tick skipped — scraper already running.")
                    return

            # Merge all due categories into one scraper call
            merged_cats: list = []
            due_keys:    list = []
            for key, cats in to_scrape:
                due_keys.append(key)
                merged_cats.extend(cats)
            merged_cats = list(dict.fromkeys(merged_cats))

            # Mark last_run before launching (prevents double-fire)
            for key in due_keys:
            ...
            print(f"[SCHEDULER] Firing scraper for cats={merged_cats} mode={publish_mode}")
            threading.Thread(
                target=_bg_scraper,
...
def _scheduler_loop(app, interval_seconds=60):
    import time
    print(f"[SCHEDULER] Loop started (tick every {interval_seconds}s).")
    while True:
...
def init_scheduler(app):
    """
    Call this once from create_app() after the app is fully configured.
    Starts a single daemon thread that ticks every 60 s.
    """
...
# ════════════════════════════════════════════════════════════════
#  SCHEDULE FORM PARSING HELPER
# ════════════════════════════════════════════════════════════════

def _parse_sched_from_form(form, cat_key: str) -> str:
    """
    Read sched_<cat_key>_* fields from a Flask form and return a JSON string
    suitable for storing in the schedule_<cat_key> DB column.
    Returns '{}' if nothing is set.
    """
    stype = form.get(f"sched_{cat_key}_type", "").strip()
    if not stype:
        return "{}"
    ...
    if stype == "interval":
        return json.dumps({
            "type":           "interval",
            "interval_hours": form.get(f"sched_{cat_key}_interval_hours", "1").strip(),
            "interval_unit":  form.get(f"sched_{cat_key}_interval_unit", "hours").strip(),
        })
    elif stype == "daily":
        return json.dumps({
            "type":      "daily",
            "time_hhmm": form.get(f"sched_{cat_key}_time", "08:00").strip(),
        })
    elif stype == "weekly":
        return json.dumps({
            "type":       "weekly",
            "weekly_day": form.get(f"sched_{cat_key}_weekly_day", "monday").strip(),
            "time_hhmm":  form.get(f"sched_{cat_key}_weekly_time", "08:00").strip(),
        })
    return "{}"
...
# ════════════════════════════════════════════════════════════════
#  MANUAL REFRESH (immediate scrape trigger from UI)
# ════════════════════════════════════════════════════════════════
...
def user_settings():
    ...
    if request.method == "POST":
        try:
            # Parse per-category schedules from form
            ...
            cursor.execute(
                ...
                INSERT INTO user_settings
                  (id, publish_mode, content_categories,
                   sync_all_schedules,
                   schedule_all, schedule_agricultural, schedule_weather,
                   schedule_financial, schedule_energy, schedule_global,
    ...
    # GET: load settings and deserialise schedule JSON for template
    try:
        cursor.execute("SELECT * FROM user_settings WHERE id=1")
        settings = cursor.fetchone() or {}
    except Exception:
    ...
    # Expose parsed schedule dicts to template via settings.get('schedule_X')
    for cat in ("all", "agricultural", "weather", "financial", "energy", "global"):
        col = f"schedule_{cat}"
        raw = (settings.get(col) or "").strip()
        try:
            settings[col] = json.loads(raw) if raw else {}
        except Exception:
            settings[col] = {}
