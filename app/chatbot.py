...
# ─── Weather synonym/alias mapping ────────────────────────────────────────────
# Maps various ways users ask about weather → normalised "weather" intent
WEATHER_TERMS = [
    # Direct weather words
    "weather", "forecast", "conditions", "climate",
    # Temperature synonyms
    "temperature", "temp", "hot", "cold", "heat", "warmth", "cool", "chilly", "freezing",
    "how hot", "how cold", "degree", "degrees", "celsius", "fahrenheit",
    # Precipitation
    "rain", "rainfall", "raining", "drizzle", "shower", "precipitation",
    "snow", "snowfall", "hail", "sleet",
    # Wind / atmosphere
    "wind", "wind speed", "windy", "gust", "storm", "thunder", "lightning",
    "humid", "humidity", "fog", "foggy", "mist", "misty",
    # Sky conditions
    "sunny", "sunshine", "cloudy", "overcast", "clear sky",
    # Contextual phrases
    "should i carry umbrella", "should i bring umbrella", "need umbrella",
    "should i wear jacket", "dress warmly", "what to wear",
    "is it raining", "will it rain",
]

# Patterns that extract city from weather-like queries
WEATHER_CITY_PATTERNS = [
    r"(?:weather|forecast|conditions?|temperature|temp|rainfall|rain|humidity|wind|fog)\s+(?:in|of|at|for)\s+([A-Za-z][A-Za-z\s]{1,30}?)(?:\s+(?:today|now|currently|right now)|[?!.,]|$)",
    r"(?:how(?:'s| is)(?: the)?|what(?:'s| is)(?: the)?)\s+(?:weather|temperature|temp|forecast|rain|humidity)\s+(?:in|at|of)\s+([A-Za-z][A-Za-z\s]{1,30}?)(?:[?!.,]|$)",
    r"([A-Za-z][A-Za-z\s]{1,20}?)(?:'s|'s)\s+(?:weather|temperature|temp|rainfall|humidity|forecast|climate)",
    r"([A-Za-z][A-Za-z\s]{1,20}?)\s+(?:weather|temperature|temp|rainfall|rain|humidity|forecast)",
    r"(?:temperature|temp|rainfall|rain|weather|climate|humidity|forecast)\s+(?:of|in|at)?\s+([A-Za-z][A-Za-z\s]{1,30}?)(?:[?!.,]|$)",
    r"(?:weather|temperature|forecast) (?:in|of|at|for)\s+([A-Za-z ]+?)(?:\s+today|\s+now|[?!.,]|$)",
    r"(?:temperature|temp|forecast) (?:in|of|at|for)\s+([A-Za-z ]+?)(?:[?!.,]|$)",
    r"how(?:'s| is) (?:the )?weather (?:in|at|of)\s+([A-Za-z ]+?)(?:[?!.,]|$)",
]

# ─── Commodity synonym/alias mapping ─────────────────────────────────────────
# Maps user-typed variants → canonical commodity name used in lookups
COMMODITY_SYNONYMS = {
    # Grains
    ...
    "bajra": "bajra", "pearl millet": "bajra", "millet": "bajra",
    # Pulses
    "moong": "moong", "green gram": "moong", "mung": "moong",
    "urad": "urad", "black gram": "urad", "urad dal": "urad",
    "arhar": "arhar", "tur": "arhar", "toor": "arhar", "pigeon pea": "arhar", "tur dal": "arhar",
    "chana": "gram", "gram": "gram", "chickpea": "gram", "Bengal gram": "gram",
    "lentil": "lentil", "masoor": "lentil", "red lentil": "lentil",
    # Vegetables
    ...
    "garlic": "garlic", "lahsun": "garlic", "lasun": "garlic",
    "chilli": "chilli", "mirchi": "chilli", "chili": "chilli", "red chilli": "chilli",
...
# Keywords in a message that signal the user wants commodity price (not just crop mention)
COMMODITY_PRICE_TRIGGERS = [
...
def detect_intent(message: str) -> str:
    ...
    # Weather intent — check all synonym terms
    if _is_weather_query(msg):
        return "weather"

    # Commodity price intent — explicit price triggers first
    if any(k in msg for k in COMMODITY_PRICE_TRIGGERS):
        return "commodity_specific"

    # Commodity by name — resolve synonyms
    resolved = _resolve_commodity_synonym(msg)
    if resolved:
        return "commodity_specific"
    ...
    # Commodities overview (generic)
    if any(k in msg for k in ["commodity","commodities","agmarknet","mandi","market prices","all prices"]):
        return "commodities"
...
def _is_weather_query(msg: str) -> bool:
    """Check whether the message is about weather using synonym list."""
    for term in WEATHER_TERMS:
        if term in msg:
            return True
    # Also handle patterns like "Chandivali's temperature"
    if re.search(r"[A-Za-z]+'s\s+(?:temperature|temp|weather|forecast|rainfall|humidity|climate)", msg, re.IGNORECASE):
        return True
    return False
...
def _resolve_commodity_synonym(msg: str) -> str:
    ...
    Returns empty string if no commodity synonym is found.
    """
    # Try longest match first to avoid 'urad' matching inside 'turmeric'
    sorted_synonyms = sorted(COMMODITY_SYNONYMS.keys(), key=len, reverse=True)
    for syn in sorted_synonyms:
        # Word-boundary match
        if re.search(r'\b' + re.escape(syn) + r'\b', msg, re.IGNORECASE):
...
def extract_city_name(message: str) -> str:
    ...
    # Fallback: strip all known weather/filler words
    cleaned = re.sub(
        ...
        r'get|city|degree|degrees|celsius|fahrenheit|conditions|climate|'
        r'sunny|cloudy|hot|cold|warm|windy)\b',
        '', msg, flags=re.IGNORECASE)
    # Handle possessives like "Chandivali's"
...
def extract_commodity_name(message: str) -> str:
    ...
    # First try synonym resolution — returns canonical name
    resolved = _resolve_commodity_synonym(msg.lower())
    if resolved:
        return resolved.title()

    # Explicit price pattern extraction
    patterns = [
        r"price of\s+([A-Za-z ]+?)(?:\s+today|\s+price|[?!.,]|$)",
        r"(?:rate|cost|mandi (?:price|rate)) of\s+([A-Za-z ]+?)(?:[?!.,]|$)",
        r"current (?:price|rate) of\s+([A-Za-z ]+?)(?:[?!.,]|$)",
...
def get_commodities_overview_response():
    ...
    all_options = _fetch_commodity_list()
    if not all_options:
        return {"status":"success","type":"commodities_overview",
                "reply":"⚠️ Commodity data unavailable right now. Try again later.","items":[]}

    # Pick a representative set of common commodities
    TARGET_CROPS = ["Wheat","Rice","Onion","Potato","Tomato","Maize","Soyabean",
...
def get_commodity_price_response(commodity_name: str = ""):
    ...
    resolved_synonym = _resolve_commodity_synonym(commodity_name.lower())
    if resolved_synonym:
        commodity_name = resolved_synonym
